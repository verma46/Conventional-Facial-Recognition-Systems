...
img = cv2.imread('Ballu.jpg')        # -------------->>>>>>>>>>>>>>>>>>  The Image to be checked
...
for (x, y, w, h) in faces:
    ...
    for _ in range(54):
        recog = cv2.face.LBPHFaceRecognizer_create(radPix)     # creating LBPH FACE RECOGNISER
    ...
    nei_conf = []
    for _ in range(13):
        recog = cv2.face.LBPHFaceRecognizer_create(radPixel, neighbour)  # creating FACE RECOGNISER
