...
for (x, y, w, h) in faces:
    ...
    for _ in range(20):
        try:
            recog = cv2.face.FisherFaceRecognizer_create(Lev)  # creating Fischer FACE RECOGNISER
...
#         Lev = Lev + 1
    ...
    fig = plt.gcf()
    fig.canvas.manager.set_window_title('RESULTS FOR FACE ' + str(face_number))
    #plt.gcf().canvas.set_window_title('RESULTS FOR FACE ' + str(face_number))
    plt.subplot(2, 1, 1)
    plt.plot(fish_ID)
